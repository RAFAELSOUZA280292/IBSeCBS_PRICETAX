...
def test_cesta_basica_nacional():
    ...
    assert code == expected_code, f"❌ FALHOU: Esperado {expected_code}, obtido {code}"
    assert "Cesta Básica Nacional" in msg, f"❌ FALHOU: Mensagem incorreta: {msg}"
...
def test_reducao_60_alimentos():
    ...
    assert code == expected_code, f"❌ FALHOU: Esperado {expected_code}, obtido {code}"
    assert "Redução 60%" in msg, f"❌ FALHOU: Mensagem incorreta: {msg}"
    assert "Anexo VII" in msg, f"❌ FALHOU: Deve mencionar Anexo VII: {msg}"
...
def test_reducao_60_essencialidade():
    ...
    assert code == expected_code, f"❌ FALHOU: Esperado {expected_code}, obtido {code}"
    assert "Redução 60%" in msg, f"❌ FALHOU: Mensagem incorreta: {msg}"
    assert "essencialidade" in msg, f"❌ FALHOU: Deve mencionar essencialidade: {msg}"
...
def test_operacao_nao_onerosa():
    ...
    assert code == expected_code, f"❌ FALHOU: Esperado {expected_code}, obtido {code}"
    assert "não onerosa" in msg, f"❌ FALHOU: Mensagem incorreta: {msg}"
...
def test_prioridade_regime_sobre_cfop():
    ...
    # Validar
    expected_code = "200003"
    assert code == expected_code, f"❌ FALHOU: Regime IVA deve ter prioridade! Esperado {expected_code}, obtido {code}"
...
def test_cfop_invalido():
    ...
    # Validar
    assert code == "", f"❌ FALHOU: Código deveria ser vazio, obtido {code}"
    assert "Informe o CFOP" in msg, f"❌ FALHOU: Mensagem incorreta: {msg}"
...
def run_all_tests():
    ...
    if failed == 0:
        ...
        print(f"\n⚠️ {failed} TESTE(S) FALHARAM - REVISAR!")
